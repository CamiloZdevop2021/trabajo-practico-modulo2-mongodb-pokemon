...
class PokemonRepository { 
    ...
    // Método para obtener todos los Pokemones
    async getAll() {
        try {
            // Obtiene todos los Pokemnones desde la base de datos
            const pokemones = await Pokemon.find();
            return pokemones;
        } catch (error) {
            console.error("Error al obtener tu Pokémon:", error);
            throw new Error('Error al obtener tu Pokémon: ' + error.message);
        }
    } 
    async getPokemon(id) {
        try {
